...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dir = std::env::args_os()
        .nth(1)
        .ok_or("usage: folder_memory <private-index-copy>")?;
    ...
    let cpu = thread_cpu();
    let paths = vec!["/home".to_owned(), "/mnt/depo".to_owned()];
    let report = index.subtree_sizes(&paths)?;
    ...
    let after = memory();
    println!(
        "folders: {:.1} ms wall / {:.1} ms CPU · anonymous {:.1} MiB before, {:.1} MiB peak, {:.1} MiB after · {} segments",
        millis(wall),
        millis(cpu),
