...
let rule;
// The rule is a template; the installer writes the account in. The test stands in for it.
vm.runInNewContext(fs.readFileSync('packaging/50-scour-service.rules', 'utf8').replace(/@USER@/g, 'alice'), {
...
test('only the named account can control the named service lifecycle', () => {
    for (const verb of ['start', 'stop', 'restart']) {
        assert.equal(check('alice', 'scour.service', verb), 'yes');
...
test('editing, transient units and other administrative actions remain gated', () => {
    ...
    for (const action of ['manage-unit-files', 'reload-daemon', 'set-environment'])
        assert.equal(check('alice', 'scour.service', 'start', 'org.freedesktop.systemd1.' + action), undefined);
    assert.equal(check('alice', 'scour.service', 'start', 'org.freedesktop.policykit.exec'), undefined);
});
